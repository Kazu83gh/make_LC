...
//     // 該当マーカーの既存の点滅のみを停止
//     if (blinkIntervals[markerId]) {
//         clearInterval(blinkIntervals[markerId]);
//     }
...
//     // 該当マーカーの点滅を開始
//     blinkIntervals[markerId] = setInterval(function() {
//         if (marObj.visibility === "visible") {
//             marObj.visibility = "hidden";
//         } else {
//             marObj.visibility = "visible";
...
//         if (blinkIntervals[markerId]) {
//             clearInterval(blinkIntervals[markerId]);
//             delete blinkIntervals[markerId];
//         }
//     } else {
//         // 全てのマーカーを停止（従来の動作）
//         for (let id in blinkIntervals) {
	    ...
	    if (probanaAjax.readyState == 4) {
	        if (probanaAjax.status == 200) {
				...
				for (let i = 0; i < selected.length; i++) {
					...
					// let markerId = i === 0 ? "" : i.toString();
    				// moveMarkerToRaDecId(selected[i][0], selected[i][1], markerId); // マーカーを移動
