...
function showPopup(evt)
{
	...
	if(document.all){
		...
		// 右クリックした位置のra,decを取得　20250819 K.Takagi
		parent.mainframe.mouseMove = 0;
        parent.mainframe.getImgStatus();     // 画像サイズ情報を取得
        parent.mainframe.figureXY(evt);      // 右クリック位置の座標を正規化
        parent.mainframe.mollwide2polar();   // ra,decに変換

		// console.log("ra: " + parent.mainframe.alpha + ", dec: " + parent.mainframe.delta);
		parent.mainframe.migiClickRa = parent.mainframe.alpha; //右クリックした位置(ra)を格納
		parent.mainframe.migiClickDec = parent.mainframe.delta; //右クリックした位置(dec)を格納
	}
...
function flux(){
    var result = '<TABLE border=1 cellspacing=0 cellpadding=3 valign="center" bordercolor="red" bgcolor="orange" width=120>';
    result += '<TR><TD onClick="" onMouseOver="setColor(this, true)" onMouseOut="setColor(this, false)">';
    ...
    result += '<TR><TD onClick="" onMouseOver="setColor(this, true)" onMouseOut="setColor(this, false)">';
    result += '<FONT size="-1" >test1</FONT></TD></TR>';
    result += '<TR><TD onClick="hidariClick()" onMouseOver="setColor(this, true)" onMouseOut="setColor(this, false)">';
