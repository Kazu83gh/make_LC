...
async function printLightCurve(x, y) {
  ...
  var pathArray = [
    "M" + String(Math.round(x)) + ",-100",
    "V1000",
    "M-100," + String(Math.round(y)),
    "H1000",
  ];
...
function figureXY(evt) {
  ...
  if (
    x > leftMargin &&
    x < orgWidth - rightMargin &&
    y > topMargin &&
    y < orgHeight - bottomMargin
  ) {
    //余白をクリックしていない時
...
function mollwide2polar() {
  ...
  if (NX * NX + 4 * NY * NY - 1 <= 0) {
    ...
    mollwidePhi = Math.asin(
      (2 * figureTheta) / Math.PI + Math.sin(2 * figureTheta) / Math.PI
    );
...
function gal2equ() {
  // 銀河座標から赤道座標へのオイラー角による変換
  ex = -0.054876 * gx + 0.494109 * gy + -0.867666 * gz;
  ey = -0.873437 * gx + -0.44483 * gy + -0.198077 * gz;
  ez = -0.483835 * gx + 0.746982 * gy + 0.455984 * gz;
}

function polar2mollwide() {
  //ニュートン法によりθを近似的に求める
  ...
  for (i = 1; i <= imax; i++) {
    fTheta =
      2 * theta0 + Math.sin(2 * theta0) - Math.PI * Math.sin(mollwidePhi);
...
function handleResult() {
  if (this.readyState == 4 && this.status == 200) {
    ...
    parent.leftframe.document.getElementById("catalogSt").innerHTML =
      "Catalog : OK" + "(" + CataData.length + ")";
...
function searchstar(csvData, flux) {
  ...
  for (i = 0; i < bnStar.length; i++) {
    ...
    var tarm1 =
      Math.sin(Delta) * Math.cos(Alpha) * Math.sin(DeltaP) * Math.cos(AlphaP);
    var tarm2 =
      Math.sin(Delta) * Math.sin(Alpha) * Math.sin(DeltaP) * Math.sin(AlphaP);
    ...
    if (angle < 1) {
      //20191206書き換え angle<2をangle<1に変更
...
function resultWrite() {
  if (pointSta == 1 && marginSta == 1) {
    ...
    for (i = 0; i < nStar.length; i++) {
      nresult +=
        '<a href="http://maxi.riken.jp/pubdata/v6l/' +
        nStar[i][0] +
        '/" target="_blank">P</a>,';
      nresult +=
        '<a href="http://maxi.riken.jp/pubdata/v6l.rkn/' +
        nStar[i][0] +
        '/" target="_blank">R</a> ';
      nresult +=
        '<font onClick="parent.mainframe.cataPointSarch(' +
        i +
        ')">' +
        nStar[i][1] +
        "<br>(" +
...
function deleteDisplay() {
  //マウスを動かした時、カーソル機能の表示を消す
  var curObj = parent.mainframe.document.getElementById("myCursor").style;
  curObj.visibility = "hidden";
}

function onmousedownFunctions(evt) {
  //クリックした時に呼び出す関数たち
  getMouseXY(evt);
  getMouseMoveXY(evt);
}
...
function FF() {
  //full frameの略。全画面表示機能。決して、Final Fantasyのことではない。
  var widthFrames;
  var heightFrames;
  var framesetCols;
  var framesetRows;
...
function loadCandidateCSV(path) {
  //突発天体の候補ファイルをpathに指定します
...
function parseCandidateCSV(str) {
  //γ線バースト？の候補の座標などが書かれたファイルを配列に収める関数
  //console.log(str);
  var lines = str.split(/\n|\r\n|\r/);
  var csvData = new Array();
...
function searchCandidate2(csvData, flux) {
  ...
  if (csvData != undefined) {
    ...
    for (i = 0; i < bnStar.length; i++) {
      ...
      if (angle < 1.5) {
        //20191206書き換え　angle<2をangle<1に変更, // 20/03/24 changed to 1.5 for FR events negoro
        angle = Math.round(angle * 100);
        angle = angle / 100;
        bnStar[i].push(angle);
        bnStar[i][2] = Math.round(bnStar[i][2] * 1000);
...
function conbineArray(array1, array2) {
  //2つの配列を結合する
...
function conbineFourArrays(array1, array2, array3, array4) {
  //4つの配列を結合する
...
async function printLightCurve(mousePositionObject) {
  ...
  if (!nearCandidate(mousePositionObject)) {
    return;
  }
  ...
  var x = alpha2,
    y = delta2;
  var promiseObject = polar2lightCurvePath(x, y, nCandidate2[0][1]);
  var lcPath = "";
...
function popupConfig() {
  delayTimeString =
    parent.leftframe.document.getElementById("popupDelay-range").value;
...
parent.leftframe.document
  .getElementById("popupDelay-range")
...
async function lightCurvePopup(mousePositionObject) {
  ...
  var popHeight = popSvg.clientHeight,
    popWidth = popSvg.clientWidth;
  var popPadding = 5;

  // popupが画面をはみ出さないように条件分岐
  styleSvg.left =
    mousePositionObject.pageX > imgWidth - popWidth - popPadding
...
function nearCandidate(mousePositionObject) {
  ...
  if (Math.abs(180 - alpha2) ** 2 + Math.abs(0 - delta2) ** 2 < 900) {
    var td = [
      "",
      "1324935584 (4orb, High, 20.84+/-9.36 mCrab)",
      180,
      0,
      "",
      "",
    ...
    nCandidate2 = [td, td].map((value, index) => {
      td[7] = value[7] + index * 10;
      return td;
    });
  }
...
async function polar2lightCurvePath(x, y, detail) {
  // console.log("do")
  ...
  // pathを作る処理の代わり
  // M-x,-500 V1000 M-500,-y H1000
  var pathArray = [
    "M" + String(-Math.round(x)) + ",-500",
    "V1000",
    "M-500," + String(-Math.round(y)),
    "H1000",
  ];
  return Promise.resolve(pathArray.join(" "));
}
